...
const RemoteManager = (() => {
  ...
  function createPanel(title, url, options = {}) {
    ...
    wrapper.className = "remote-panel remote-window-fixed";
  ...
  function openRemoteWindow(options) {
    if (!options || panels.length >= 3) {
      return null;
    }
    return createPanel(options.title || "Remote", options.url, options);
  }
  ...
  function bindRemoteLinks() {
    $$(".remote-link").forEach((link) => {
      link.addEventListener("click", (e) => {
        e.preventDefault();
        Dropdowns.close();
        addPanel({ title: link.dataset.name || "Remote", url: link.dataset.url });
      });
    });
  }
  ...
  return { init, addPanel, openRemoteWindow };
})();
